...
//********************************************************************************
    ...
    ana.looper.init(ana.events_tchain, &dilep, ana.n_events);

    bool isdata = ana.input_tree_name.Contains("data_");

    ana.cutflow.setTFile(ana.output_tfile);

    ana.cutflow.addCut("Wgt", [&]() { return (dilep.good_leptons_p4()[0].pt() > 40) and (dilep.good_leptons_p4()[1].pt() > 40); }, [&, isdata]() { return isdata ? 1. : dilep.wgt() * dilep.lepsf(); } );

    ana.cutflow.getCut("Wgt");
    ana.cutflow.addCutToLastActiveCut("EE",     [&, isdata]() { return (dilep.good_leptons_pdgid()[0] * dilep.good_leptons_pdgid()[1] == -121) and (dilep.trig_ee()) and (isdata ? dilep.pass_duplicate_ee_em_mm() : 1.) and fabs((dilep.good_leptons_p4()[0] + dilep.good_leptons_p4()[1]).mass() - 91.1876) < 15; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("EEPt10", [&, isdata]() { return (dilep.good_leptons_p4()[0] + dilep.good_leptons_p4()[1]).pt() > 10; }, UNITY);
    ana.cutflow.getCut("Wgt");
    ana.cutflow.addCutToLastActiveCut("EM",     [&, isdata]() { return (dilep.good_leptons_pdgid()[0] * dilep.good_leptons_pdgid()[1] == -143) and (dilep.trig_em()) and (isdata ? dilep.pass_duplicate_ee_em_mm() : 1.); }, UNITY);
    ana.cutflow.getCut("Wgt");
    ana.cutflow.addCutToLastActiveCut("MM",     [&, isdata]() { return (dilep.good_leptons_pdgid()[0] * dilep.good_leptons_pdgid()[1] == -169) and (dilep.trig_mm()) and (isdata ? dilep.pass_duplicate_ee_em_mm() : 1.) and fabs((dilep.good_leptons_p4()[0] + dilep.good_leptons_p4()[1]).mass() - 91.1876) < 15; }, UNITY);
    ana.cutflow.addCutToLastActiveCut("MMPt10", [&, isdata]() { return (dilep.good_leptons_p4()[0] + dilep.good_leptons_p4()[1]).pt() > 10; }, UNITY);

    // Print cut structure
    ana.cutflow.printCuts();

    // Histogram utility object that is used to define the histograms
    ana.histograms.addHistogram("Mll", 180, 0, 250, [&]() { return (dilep.good_leptons_p4()[0] + dilep.good_leptons_p4()[1]).mass(); } );
    ana.histograms.addHistogram("Ptll", 180, 0, 250, [&]() { return (dilep.good_leptons_p4()[0] + dilep.good_leptons_p4()[1]).pt(); } );
    ana.histograms.addHistogram("MllZ", 180, 60, 120, [&]() { return (dilep.good_leptons_p4()[0] + dilep.good_leptons_p4()[1]).mass(); } );
    ana.histograms.addHistogram("DPhill", 180, 0, 3.1416, [&]() { return fabs(RooUtil::Calc::DeltaPhi(dilep.good_leptons_p4()[0], dilep.good_leptons_p4()[1])); } );
    ana.histograms.addHistogram("DEtall", 180, 0, 5, [&]() { return fabs(RooUtil::Calc::DeltaEta(dilep.good_leptons_p4()[0], dilep.good_leptons_p4()[1])); } );
    ana.histograms.addHistogram("Pt0", 180, 0, 250, [&]() { return dilep.good_leptons_p4()[0].pt(); } );
    ana.histograms.addHistogram("Pt1", 180, 0, 250, [&]() { return dilep.good_leptons_p4()[1].pt(); } );
    ana.histograms.addHistogram("Eta0", 180, -2.5, 2.5, [&]() { return dilep.good_leptons_p4()[0].eta(); } );
